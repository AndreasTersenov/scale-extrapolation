"""NIGHT-3 starlet trained leg per arm tag (~/wl-challenge-env; frozen
scorer, leg 15). Usage: night3_starlet.py <tag> — reads
stage3_<tag>_final.npz, writes starlet_l1_<tag>.json."""
import json
import os
import sys
...
tag = sys.argv[1]
G = np.load(os.path.join(RES, f"stage3_{tag}_final.npz"))
...
with open(os.path.join(RES, f"starlet_l1_{tag}.json"), "w") as fh:
    json.dump(meas, fh, indent=1)
print(f"[{tag}] starlet gen_A: "
