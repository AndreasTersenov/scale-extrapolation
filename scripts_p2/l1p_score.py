"""L1' SCORE (env.sh; pre-statement 67d7304, R39). Two modes:

  --leg canary : e2e couplings of the canary stream octaves 1-4 vs sandbox
                 truth (standing bars context). KILL iff var_slope rel err
                 > 50% at any scored octave (2-4). Writes l1p_canary_score.json.
                 The main job may be submitted ONLY on 'CANARY PASS'.
  --leg main   : the binding order — identity gates recorded, then A2
                 (pooled C over the 3 adjudicating streams, written+printed
                 BEFORE any peak number is computed), then peaks vs the A3
                 reference, per-stream + pooled, must-not-regress set
                 (marginals catastrophe, starlet jsons if present, parity_T,
                 nn WATCHED), then the mechanical branch. Writes
                 l1p_verdict.json.

Branch rules verbatim from the pre-statement (weights rec/exec 10/10,
35/40, 35/30 A2-split, 10/10, 10/10): FLIPPED (either pooled ci95 entirely
< 0) -> REGRESSED (any must-not-regress failure) -> CURED (both ci95
include 0) -> at-bar/mixed (worse governs, one fresh-PRNG disambiguation
licensed) -> IMPROVED (both Delta >= 2) -> NULL (A2 splits WITH/WITHOUT
transfer). A3: r = (+15.290, +12.474)%, SE_ref = (3.12, 2.80)%.
"""
...
A2_BASE, A2_THRESH = 0.7237, 0.7551
...
def canary():
    ...
    gen = [np.asarray(f, np.float64) for f in
           np.load(os.path.join(RES, "l1p_canary_gen.npz"))["gen_canary"]]
    e2e = couplings(gen, [1, 2, 3, 4], n_boot=200, seed=0)
    rows, kill = {}, []
    for j in (1, 2, 3, 4):
        rows[str(j)] = {}
        for metric in ("var_slope", "kurtosis"):
            if str(j) not in TRUTH:
                continue
            ...
            rows[str(j)][metric] = {"value": v, "truth": t, "rel_err": rel,
    ...
    out = {"rows": rows, "kill": kill, "verdict": verdict,
           "replay_gate": json.load(open(os.path.join(
               RES, "l1p_canary_sample.json")))["replay_gate"]}
    with open(os.path.join(RES, "l1p_canary_score.json"), "w") as f:
...
def main_leg():
    ...
    out = {"identity_gates": {
        ...
        "main": json.load(open(os.path.join(
            RES, "l1p_main_sample.json")))["replay_gate"]}}

    # ---- A2 FIRST (written before any peak number exists) -------------------
    c_pool, se_pool = stack_coloring(adj, 1, seed=1000)
    per_stream = {k: stack_coloring([np.asarray(f, np.float64)
                                     for f in GEN[k]], 1, seed=1001 + i)
                  for i, k in enumerate(("adj1", "adj2", "adj3",
                                         "ablation_oct1meas"))}
    transfer = bool(c_pool >= A2_THRESH and
                    (c_pool - A2_BASE) >= 3 * se_pool)
    out["a2"] = {"C_pooled": c_pool, "SE_pooled": se_pool,
                 "threshold": A2_THRESH, "base": A2_BASE,
                 "sigma_over_base": (c_pool - A2_BASE) / se_pool,
                 "per_stream": {k: {"C": v[0], "SE": v[1]}
                                for k, v in per_stream.items()},
                 "TRANSFER": transfer}
    with open(os.path.join(RES, "l1p_a2.json"), "w") as f:
        json.dump(out["a2"], f, indent=1)
    print(f"A2: C_pooled={c_pool:.4f}±{se_pool:.4f} "
          f"(+{(c_pool - A2_BASE) / se_pool:.1f}σ over 0.7237; "
          f"need >=0.7551 at 3σ) -> "
          f"{'TRANSFER' if transfer else 'NO-TRANSFER'}")
    for k, v in per_stream.items():
    ...
    for i, k in enumerate(("adj1", "adj2", "adj3", "ablation_oct1meas")):
        gen = [np.asarray(f, np.float64) for f in GEN[k]]
        peaks[k] = {nu: bootstrap_excess(gen, real, float(nu),
                                         seed=1200 + 10 * i + j)
                    for j, nu in enumerate(("2.5", "3.0"))}
    peaks["edge"] = {nu: bootstrap_excess(
        [np.asarray(f, np.float64) for f in GEN["edge"]], sd_real, float(nu),
        seed=1300 + j) for j, nu in enumerate(("2.5", "3.0"))}
    out["peaks"] = peaks
    delta, percat, atbar = {}, {}, {}
    for nu in ("2.5", "3.0"):
    ...
    starlet = {}
    for tag in ("trained", "edge"):
        p = os.path.join(RES, f"starlet_l1_l1p_{tag}.json")
        if os.path.exists(p):
            m = json.load(open(p))
            starlet[tag] = {a: m["checks"][a]["all_scored_pass"]
                            for a in m.get("checks", {})}
    gates_ok = all(g["corr_min"] >= 0.99 for g in out["identity_gates"].values())
    starlet_ok = all(all(v.values()) for v in starlet.values()) if starlet \
        else None
    ...
    if flipped:
        ...
        worse = "NULL"
        branch = {"branch": "MIXED/AT-BAR", "governing": worse,
        ...
        branch = {"branch": "N-NULL-WITH-TRANSFER" if transfer
                  else "N-NULL-WITHOUT-TRANSFER"}
    branch["a2_transfer"] = transfer
    out["adjudication"] = branch

    with open(os.path.join(RES, "l1p_verdict.json"), "w") as f:
        json.dump(out, f, indent=1)
    for k in ("pooled", "adj1", "adj2", "adj3", "ablation_oct1meas", "edge"):
        row = "  ".join(
            f"nu={nu}: {peaks[k][nu]['excess']:+.2%}±{peaks[k][nu]['se']:.2%}"
            for nu in sorted(peaks[k]))
        print(f"peaks {k:>18}: {row}")
    print(f"delta: nu2.5 {delta['2.5']:.2f} nu3.0 {delta['3.0']:.2f} "
          f"(IMPROVED needs both >=2)")
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    canary() if args.leg == "canary" else main_leg()
