...
steps = [
    ...
    (0.1, "PHASE 1d — the audit IS the asset (reshape memo with Andreas)", "Generator frozen (measured compounding cap; tested negative). The pivot: (1) the inverted\npilot CAUGHT every known failure with no extrapolated-octave truth (P-edge pass, self-consistency\nz=7.9, scattering 98-100% of channels); (2) downstream demo: P(k) passes while the PEAK function\nis tilted +14sigma/-12sigma (spurious graininess + missing extremes); (3) the self-similar control: same\narchitecture, no drift -> 2.5% residual (vs 39-65%) -- the cap is the FIELD'S DRIFT, not the\narchitecture. Kill-tests (Gate 0) pending in a parallel session; RESHAPE-MEMO.md awaits decision.\nFigures: selfsim_control.png, downstream_peaks.png, pilot_validation.png.", GREEN, "memo->Andreas"),
]
