...
steps = [
    ...
    (0.1, "4a CONFIRMED (augmentation frozen in) -> 4b' FAILED at the lever bar", "4a: collapse ELIMINATED by 8x data -> the memorization law is causally confirmed.\n4b' (conditioning corruption, anti-compounding): the heads got BETTER (own ceilings now ~at\nreal, kurtosis up) but end-to-end didn't move -- s_gen=0 sampling never engages the trained\nrobustness. Binding failure = recursion compounding at octave 2. Stopped at the readout;\nzero-training follow-up (re-sample s=0.3 ckpts with s_gen>0) named for the reconvene.\nFigures: readout_4bp.png, signature_4a.png.", "#D55E00", "readout->stop"),
]
